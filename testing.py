...
def to_screen(gx, gy, z=0):
    ...
    # FIXED: Inverted Y calculation. 
    # Increasing gy (Forward) now moves visually UP the screen.
...
class Lane:
    def __init__(self, y_index, type_id):
        ...
        if self.type == 'grass':
            ...
            if random.random() < 0.1: self.coin = Coin(random.randint(-5, 5), self.y)
             ...
             self.train_timer = random.randint(0, 200) 
    ...
    def update(self):
        ...
        if self.type == 'road' and self.timer > 140:
            ...
            self.timer = 0
        elif self.type == 'water' and self.timer > 90: 
            self.objects.append(Object(-30 if self.direction == 1 else 30, self.y, self.speed * self.direction, "log"))
            self.timer = 0
        elif self.type == 'rail':
            self.train_timer += 1
            if self.train_timer > 400: self.train_warning = True
            if self.train_timer > 460:
                self.objects.append(Object(-50, self.y, 1.8, "train"))
                self.train_timer = 0
                self.train_warning = False
...
class Game:
    ...
    def update(self):
        ...
        if not self.is_moving:
            ...
            # FIXED: UP moves character to Top/Forward
            elif keys[pygame.K_UP]: dy = 1
            # FIXED: DOWN moves character to Bottom/Backward
            ...
            if self.jump_anim >= 1:
                ...
                # SCORE LOGIC: Only update if new position is greater than current max score
                if self.py > self.score: 
                    self.score = int(self.py)
    ...
    def draw(self):
        ...
        # FIXED: Reversed drawing order. 
        # Since Forward is now Up/Background, we must draw High Y first.
        start_y = int(self.py) - 12
        end_y = int(self.py) + 15
        ...
        # FIXED: Sort objects back-to-front (High depth first)
        render_list.sort(key=get_depth, reverse=True)
